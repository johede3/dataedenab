...
const PricingColumn: React.FC<Props> = ({ tier, highlight }: Props) => {
  ...
  return (
    <div
      className={clsx("mx-auto w-full max-w-sm rounded-xl border border-gray-200 bg-white lg:max-w-full", {
        "shadow-lg": highlight,
      })}
    >
        ...
        <ul className="mb-8 space-y-4">
          {features.map((feature, index) => (
            <li key={index} className="flex items-center">
              <BsFillCheckCircleFill className="text-secondary mr-2 h-5 w-5" />
